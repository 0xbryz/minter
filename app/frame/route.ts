...
export async function GET(req: NextRequest, res: NextResponse) {
  try {
    const frameMetadata = await fdk.getFrameMetadata({
      ...
      aspect_ratio: "1:1",
      cid: "QmUpB8CMutJXhMhkEFxyUTLqbMXBphoBzwMuyNuDzKq1Qv",
    });
  ...
  if (typeof balance === "number" && balance !== null && balance < 1) {
    try {
      ...
      const frameMetadata = await fdk.getFrameMetadata({
        ...
        aspect_ratio: "1:1",
        cid: "QmcEsnjCUrPTyF89jtwYtuhcchsBG4ajtEtoE8CcDuRySi",
      });
      return new NextResponse(frameMetadata);
    } catch (error) {
      console.log(error);
    ...
    const frameMetadata = await fdk.getFrameMetadata({
      ...
      buttons: [{ label: "Learn How to Make This", action: "post_redirect" }],
      aspect_ratio: "1:1",
      cid: "QmR6btLwaGcUGuGQMZiudFherZjPqLorJYscTbwCCFfzsH",
    });
    return new NextResponse(frameMetadata);
  }
}
